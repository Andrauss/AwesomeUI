...
public class FadeOutDownTransition extends CachedTimelineTransition {
    ...
    public FadeOutDownTransition(final Node node, Double translateY) {
        super(node, new Timeline(
                new KeyFrame(Duration.millis(0),
                        new KeyValue(node.opacityProperty(), 1, WEB_EASE),
                        new KeyValue(node.translateYProperty(), translateY, WEB_EASE)
                ),
                new KeyFrame(Duration.millis(1000),
                        new KeyValue(node.opacityProperty(), 0, WEB_EASE),
                        new KeyValue(node.translateYProperty(), translateY + 20, WEB_EASE)
                )
        ));
        setCycleDuration(Duration.seconds(1));
        setDelay(Duration.seconds(0.2));
